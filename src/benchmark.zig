...
pub fn main() !void {
    ...
    for (0..warmup_runs + measure_runs) |run| {
        ...
        const array2 = root_value2.getObjectFromPath(&path) orelse return error.MissingField;
        for (0..array2.getArrayLength()) |i| {
            const user_name_path = [_][:0]const u8{ "user", "screen_name" };
            const text = array2.getArrayValueAt(i).?.getObjectFromPath(&user_name_path) orelse return error.MissingField;
            const x = [_]f64{ 1, 2, 3, 4, 5 };
            _ = try text.setArray(&x);
            _ = text.getString() orelse return error.TypeError;
        }
